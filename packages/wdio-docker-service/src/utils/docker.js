...
class Docker extends EventEmitter {
     ...
     */
    run() {
        this.debug && this.logger.info(`Docker command: ${this.dockerRunCommand}`)
    ...
    _reportWhenDockerIsRunning() {
        ...
        return Docker.delay(startDelay)
            .then(() => new Promise((resolve, reject) => {
                ...
                this.debug && this.logger.info('Polling for healthcheck...')
                ...
                const poll = () => {
                    Ping(url)
                        .then(() => {
                            ...
                            pollstatus = null
                            this.debug && this.logger.info('Healthcheck succeeds')
                        })
                        .catch((err) => {
                            attempts++
                            if (attempts >= maxRetries) {
                                ...
                                pollstatus = null
                                reject(err)
                                this.debug && this.logger.info(`Healthcheck failed after ${ attempts } attempts`)
                                return
                            }
